    ...
    for dataset_ in datasets["DatasetName"]:
        ...
        if frmirprep_uri := dataset_df.fmriprep.values[0]:
            if (sourcedata / f"{dataset_}-fmriprep").exists():
                print(f' fmriprep data already present at {sourcedata / dataset_}')
            else:
                print(f' Cloning raw data at: {sourcedata / dataset_}-fmriprep')
                api.install(path=sourcedata / f"{dataset_}-fmriprep",  source=frmirprep_uri)
